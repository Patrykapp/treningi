...
import { Camera, CameraOff, Search, PackagePlus, Flashlight, AlertTriangle, ArrowLeft, Star, Sparkles, Link as LinkIcon, Pencil } from 'lucide-react';

// Ostatnio używana zakładka. Kto skanuje, ten skanuje codziennie; kto opisuje
// posiłki zdaniem, ten nie chce co wieczór klikać w „Opisz".
const TAB_KEY = 'foodPickerTab';
...
export type Candidate = {
  ...
  servingLabel?: string | null; // opis porcji, np. „1 bułka" — włącza liczenie w sztukach
  usualGrams?: number | null;   // ile zwykle idzie tego na ten posiłek
  unit: 'g' | 'ml';
  source: 'OWN' | 'SEED' | 'OFF';
};
...
type CatalogRow = {
  ...
  servingG: number | null; servingLabel?: string | null; usualGrams?: number | null;
  source: string; usageCount: number;
  unit?: string; isFavorite?: boolean; recipe?: string | null; edited?: boolean;
};
...
/** Ilość dla przycisku „+N": własny zwyczaj, a jak go nie ma — typowa porcja. */
function quickGrams(c: Candidate): number {
  if (c.usualGrams && c.usualGrams > 0) return Math.round(c.usualGrams);
  return c.servingG && c.servingG > 0 ? Math.round(c.servingG) : 100;
}

/** Ilość „na start" po wybraniu produktu: własny zwyczaj bije typową porcję. */
function initialAmount(c: Candidate): { mode: 'unit' | 'piece'; grams: string; pieces: string } {
  const per = c.servingG && c.servingG > 0 ? c.servingG : 0;
  const usual = c.usualGrams && c.usualGrams > 0 ? c.usualGrams : 0;
  const base = usual || (per > 0 && per <= 500 ? Math.round(per) : 100);

  // Sztuki mają sens tylko przy porcji opisanej słowem („1 bułka", „1 kromka").
  // Bez takiego opisu „1,7 porcji" nie jest łatwiejsze niż 170 g.
  if (per > 0 && c.servingLabel) {
    const n = base / per;
    const rounded = Math.round(n * 2) / 2; // do połówek
    if (rounded > 0 && Math.abs(n - rounded) < 0.12) {
      return { mode: 'piece', grams: String(Math.round(rounded * per)), pieces: String(rounded).replace('.', ',') };
    }
  }
  return { mode: 'unit', grams: String(base), pieces: per > 0 ? '1' : '' };
}
...
export function FoodPicker({
  ...
  // Liczenie w sztukach: dwie bułki zamiast 120 g. Przelicznik to servingG,
  // więc do bazy i tak trafia gramatura — sztuki są tylko sposobem wpisania.
  const [amountMode, setAmountMode] = useState<'unit' | 'piece'>('unit');
  const [piecesStr, setPiecesStr] = useState('1');
  const perPiece = picked?.servingG && picked.servingG > 0 ? picked.servingG : 0;
  const grams =
    amountMode === 'piece' && perPiece > 0
      ? Math.round((dec(piecesStr) || 0) * perPiece)
      : parseInt(gramsStr, 10) || 0;
  const [saving, setSaving] = useState(false);
  const [note, setNote] = useState('');

  // poprawianie wartości produktu w katalogu
  const [editOpen, setEditOpen] = useState(false);
  const [editForm, setEditForm] = useState({ kcal: '', protein: '', carbs: '', fat: '', serving: '' });
  ...
  // Co import faktycznie policzył — przy wyliczaniu ze składników to jedyny
  // sposób, żeby zobaczyć złe dopasowanie zanim się je zapisze.
  const [importUsed, setImportUsed] = useState<{ name: string; grams: number }[]>([]);
  ...
  // Zakładka zapamiętana z poprzedniego razu. Kamery NIE włączamy sami —
  // uruchomienie jej bez kliknięcia byłoby nadużyciem zgody na dostęp.
  useEffect(() => {
    if (!isOpen) return;
    try {
      const saved = localStorage.getItem(TAB_KEY);
      setTab(saved === 'describe' || saved === 'scan' || saved === 'new' ? saved : 'search');
    } catch {
      setTab('search'); // brak dostępu do storage
    }
  }, [isOpen]);

  /** Ręczna zmiana zakładki — tylko taka jest zapamiętywana. Przeskok na „Nowy"
   *  po nieudanym skanie to nie jest wybór użytkownika. */
  const pickTab = (t: 'search' | 'describe' | 'scan' | 'new') => {
    if (t !== 'scan') stopScan();
    setTab(t);
    try {
  ...
  const importRecipe = async () => {
    ...
    try {
      ...
      setImportUsed(Array.isArray(b.used) ? b.used : []);
    } catch {
      setNote('Brak połączenia z serwerem.');
    } finally {
      setImporting(false);
  ...
  const confirm = async () => {
    ...
    try {
      ...
      // Tryb seryjny: po zapisaniu kamera wraca sama, żeby przy rozpakowywaniu
      // zakupów lecieć kod po kodzie bez klikania.
      if (wasScan && serial) {
        lastCodeRef.current = '';
        setTab('scan');
        void startScan();
  ...
  /** Zapisanie poprawionych wartości produktu w katalogu. */
  const saveProduct = async () => {
    if (!picked?.productId) return;
    const kcal = dec(editForm.kcal);
    if (!Number.isFinite(kcal) || kcal < 0) {
      setNote('Podaj kalorie na 100 ' + picked.unit + '.');
      return;
    }
    ...
    try {
      const body = {
        kcal100: kcal,
        protein100: Number.isFinite(dec(editForm.protein)) ? dec(editForm.protein) : 0,
        carbs100: Number.isFinite(dec(editForm.carbs)) ? dec(editForm.carbs) : 0,
        fat100: Number.isFinite(dec(editForm.fat)) ? dec(editForm.fat) : 0,
        servingG: editForm.serving.trim() === '' ? null : dec(editForm.serving),
      };
      const res = await fetch(`/api/food/products/${picked.productId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!res.ok) {
        ...
        setNote(b?.error || 'Nie udało się zapisać poprawki.');
        return;
      }
      const saved: CatalogRow = await res.json();
      // Poprawka działa od teraz w przód — wpisy już w dzienniku mają własny
      // snapshot i zostają takie, jakie były w chwili jedzenia.
      setPicked(fromCatalog(saved));
      setCatalog((prev) => prev.map((c) => (c.id === saved.id ? { ...c, ...saved } : c)));
      setEditOpen(false);
      setNote('Poprawione. Skrypt bazy podstawowej już tego nie nadpisze.');
    } finally {
      ...
      {picked ? (
          ...
          <div className="flex items-start justify-between gap-2">
            ...
            {picked.productId && (
              <button
                onClick={() => {
                  setEditForm({
                    kcal: String(picked.kcal100),
                    protein: String(picked.protein100),
                    carbs: String(picked.carbs100),
                    fat: String(picked.fat100),
                    serving: picked.servingG ? String(Math.round(picked.servingG)) : '',
                  });
                  setEditOpen((s) => !s);
                }}
                className="shrink-0 flex items-center gap-1 text-xs text-blue-600 px-2 py-1"
              >
                <Pencil className="w-3.5 h-3.5" /> popraw
              </button>
            )}
          </div>
          ...
          {editOpen && (
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 space-y-2">
              <p className="text-xs text-blue-900">
                Wartości na 100 {picked.unit}. Etykieta z twojej piekarni bije wartość uśrednioną — poprawka
                zostaje na stałe i nie zniknie przy kolejnym wgraniu bazy. Wpisy już w dzienniku zostają bez zmian.
              </p>
              <div className="grid grid-cols-2 gap-2">
                {([
                  ['kcal', `Kalorie (kcal / 100 ${picked.unit})`],
                  ['protein', `Białko (g / 100 ${picked.unit})`],
                  ['carbs', `Węglowodany (g / 100 ${picked.unit})`],
                  ['fat', `Tłuszcz (g / 100 ${picked.unit})`],
                  ['serving', `Typowa porcja (${picked.unit})`],
                ] as const).map(([k, label]) => (
                  <label key={k} className="text-xs">
                    <span className="block text-gray-600 mb-0.5">{label}</span>
                    <input
                      value={editForm[k]}
              ...
              >
                {savingProduct ? 'Zapisuję…' : 'Zapisz poprawkę'}
              </button>
            </div>
          )}

          {/* Sztuki albo gramy. Przelicznik to typowa porcja, więc do dziennika
              i tak trafia gramatura — sztuki są tylko wygodniejszym wpisem. */}
          {perPiece > 0 && picked.servingLabel && (
            <div className="flex gap-2">
              {([
                ['piece', picked.servingLabel.replace(/^1\s+/, '') || 'sztuki'],
                ['unit', picked.unit === 'ml' ? 'mililitry' : 'gramy'],
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    if (m === 'piece') {
                      const n = Math.round(((parseInt(gramsStr, 10) || 0) / perPiece) * 2) / 2;
                      setPiecesStr(String(n > 0 ? n : 1).replace('.', ','));
                    } else {
                      setGramsStr(String(grams || Math.round(perPiece)));
                    }
                    setAmountMode(m);
                  }}
                  className={`flex-1 py-1.5 rounded-lg text-xs font-medium ${
                    amountMode === m ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'
                  }`}
                >
                  {label}
                </button>
          ...
          {amountMode === 'piece' && perPiece > 0 ? (
            <div className="flex items-center gap-2 flex-wrap">
              <input
                ...
                inputMode="decimal"
                value={piecesStr}
                onChange={(e) => setPiecesStr(e.target.value.replace(/[^\d.,]/g, '').slice(0, 4))}
                className="w-24 rounded-lg border border-gray-300 px-3 py-2 text-lg font-semibold"
                autoFocus
              />
              <span className="text-gray-600 text-sm">
                × {Math.round(perPiece)} {picked.unit} = <strong>{grams} {picked.unit}</strong>
              </span>
              <div className="flex gap-2 w-full">
                {['0,5', '1', '2', '3'].map((n) => (
                  <button
                    key={n}
                    onClick={() => setPiecesStr(n)}
                    className="flex-1 px-3 py-1.5 rounded-lg bg-gray-100 text-sm"
                  >
                    {n}
                  </button>
                ))}
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {picked.usualGrams ? (
                <button
                  onClick={() => setGramsStr(String(Math.round(picked.usualGrams!)))}
                  className="px-3 py-1.5 rounded-lg bg-green-50 text-green-700 text-sm"
                >
                  zwykle {Math.round(picked.usualGrams)} {picked.unit}
                </button>
              ) : picked.servingG ? (
                <button
        ...
        <div className="space-y-3">
          <div className="flex gap-1.5">
            <button onClick={() => pickTab('search')} className={tabCls('search')}>Szukaj</button>
            <button onClick={() => pickTab('describe')} className={tabCls('describe')}>Opisz</button>
            <button onClick={() => pickTab('scan')} className={tabCls('scan')}>Skanuj</button>
            <button onClick={() => pickTab('new')} className={tabCls('new')}>Nowy</button>
          </div>
                      ...
                      <li key={c.id} className="flex items-center gap-1">
                        <button onClick={() => choose(fromCatalog(c))} className="flex-1 min-w-0 text-left py-2 flex justify-between gap-3">
                          <span className="min-w-0">
                            ...
                            <span className="block text-xs text-gray-500">
                              ...
                              {c.usualGrams
                                ? ` · zwykle ${Math.round(c.usualGrams)} ${c.unit === 'ml' ? 'ml' : 'g'}`
                                : c.servingG
                                  ? ` · porcja ${Math.round(c.servingG)} ${c.unit === 'ml' ? 'ml' : 'g'}`
                                  : ''}
                            </span>
                          </span>
                        ...
                        {c.servingG || c.usualGrams ? (
                          <button
                            ...
                            title={`Dodaj ${quickGrams(fromCatalog(c))} ${c.unit === 'ml' ? 'ml' : 'g'}`}
                          >
                            +{quickGrams(fromCatalog(c))}
                          </button>
                        ) : null}
                      </li>
                    ))}
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center gap-2 text-sm text-gray-600">
                <input
                  ...
                  className="w-4 h-4"
                />
                Skanuj seryjnie — po zapisaniu kamera wraca sama
              </label>
              {scanning && (
          ...
          {tab === 'new' && (
            <div className="space-y-2">
              ...
              <div className="rounded-lg border border-blue-200 bg-blue-50 p-3 space-y-2">
                ...
                {importUsed.length > 0 && (
                  <details className="text-xs text-blue-800">
                    <summary className="cursor-pointer">Z czego to policzyłem ({importUsed.length})</summary>
                    <ul className="mt-1 space-y-0.5">
                      {importUsed.map((u, i) => (
                        <li key={`${u.name}-${i}`} className="flex justify-between gap-2">
                          <span className="min-w-0 truncate">{u.name}</span>
                          <span className="shrink-0 text-blue-600">{u.grams} g</span>
                        </li>
                      ))}
                    </ul>
                  </details>
              ...
              {/* Etykiety NAD polami, nie w podpowiedzi. Placeholder znika po
                  wpisaniu wartości i po imporcie przepisu zostawały cztery gołe
                  liczby, o których nie sposób powiedzieć, która jest którym
                  makroskładnikiem. */}
              <div className="grid grid-cols-2 gap-2">
                {([
                  ['kcal', 'Kalorie', `kcal / 100 ${formUnit} *`],
                  ['protein', 'Białko', `g / 100 ${formUnit}`],
                  ['carbs', 'Węglowodany', `g / 100 ${formUnit}`],
                  ['fat', 'Tłuszcz', `g / 100 ${formUnit}`],
                  ['serving', 'Typowa porcja', formUnit],
                  ['barcode', 'Kod kreskowy', 'opcjonalnie'],
