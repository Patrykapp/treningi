...
interface EntryRow extends NewEntryForm {
  ...
  customSets: boolean;
  bodyweight: boolean;
  // Ćwiczenie mierzone czasem zamiast serii. W formularzu trzymamy minuty,
  // do bazy idą sekundy.
  timed: boolean;
  durationMin: number;
}
...
function newEntryRow(key: string, partial: Partial<EntryRow> = {}): EntryRow {
  return {
    key, exerciseId: '', sets: 3, reps: 10, weight: 0,
    customSets: false, setsData: [], bodyweight: false, timed: false, durationMin: 0,
    ...partial,
  };
}
...
function progressionHint(last: LastResult): string {
  // Przy ćwiczeniu na czas progresją są minuty, nie kilogramy.
  if (last.durationSec && last.durationSec > 0) {
    return `spróbuj ${Math.round(last.durationSec / 60) + 2} min`;
  }
...
function formatLastResult(last: LastResult): string {
  if (last.durationSec && last.durationSec > 0) return `${Math.round(last.durationSec / 60)} min`;
...
function TreningPage() {
  ...
  const [entries, setEntries] = useState<EntryRow[]>([newEntryRow('0')]);
  ...
  const loadData = useCallback(async () => {
    ...
    if (sessionIdParam) {
      ...
      if (session && !session.error) {
        ...
        setEntries(session.entries.map((e: {
          exerciseId: string; sets: number; reps: number; weight: number; durationSec?: number | null;
          ...
          return newEntryRow(String(i), { exerciseId: e.exerciseId, sets: e.sets, reps: e.reps, weight: e.weight,
            ...
            timed: Boolean(e.durationSec && e.durationSec > 0),
            durationMin: e.durationSec ? Math.round(e.durationSec / 60) : 0 });
        }));
        return;
      }
    }
    ...
    if (templateIdParam) {
      ...
      if (tpl) {
        setEntries(tpl.entries.map((e, i) => newEntryRow(String(Date.now() + i), {
    ...
    try {
      ...
      if (saved) {
        ...
        // Szkic zapisany przed dodaniem pomiaru czasem nie ma nowych pól —
        // przepuszczamy go przez fabrykę, żeby nie powstał wiersz bez `timed`
        // i bez `durationMin` (React zgłasza wtedy niekontrolowane pole).
        if (Array.isArray(draft.entries) && draft.entries.length > 0) {
          setEntries(draft.entries.map((e: Partial<EntryRow>, i: number) =>
            newEntryRow(e.key ?? String(i), e)
          ));
        }
      }
    } catch {
      ...
      fetch(`/api/entries?exerciseId=${exId}&userId=${hintUserId}&limit=1`)
        ...
        .then(data => {
          ...
          setLastResults(prev => ({
            ...prev,
            [key]: e ? {
              sets: e.sets, reps: e.reps, weight: e.weight, durationSec: e.durationSec ?? null,
              rpe: e.rpe ?? null,
              date: e.session?.date || '',
  ...
  const copyLastWorkout = async () => {
    ...
    setEntries(last.entries.map((e: {
      ...
      const sd: SetData[] = Array.isArray(e.setsData) && e.setsData.length > 0 ? e.setsData : [];
      return newEntryRow(String(Date.now() + i), { exerciseId: e.exerciseId, sets: e.sets, reps: e.reps,
  ...
  const addEntry = () => setEntries(prev => [...prev, newEntryRow(String(Date.now()))]);
  ...
  const updateEntry = (key: string, field: keyof NewEntryForm, value: string | number) =>
    setEntries(prev => prev.map(e => {
      ...
      // Wybór ćwiczenia ustawia domyślny tryb pomiaru. Bieżnia od razu prosi
  ...
  const validateEntries = (): boolean => {
    ...
    for (const entry of entries) {
      ...
      if (entry.timed) {
        if (!entry.durationMin || entry.durationMin <= 0) {
          setToast({ message: 'Podaj czas w minutach', type: 'error' }); return false;
        }
        continue; // przy pomiarze czasem serie i ciężar nie obowiązują
      }
  ...
  const buildEntryPayload = (entry: EntryRow) => {
    if (entry.timed) {
      return {
        ...
        durationSec: Math.round(entry.durationMin * 60),
        rpe: entry.rpe,
  ...
  const loadAiPlan = () => {
    if (!aiPlan) return;
    const newEntries: EntryRow[] = aiPlan.exercises.map((ex, i) => newEntryRow(String(Date.now() + i), {
        ...
        {entries.map((entry, idx) => (
          <div key={entry.key} className="bg-white rounded-2xl p-4 space-y-3">
            ...
            <div className="flex gap-4 items-center flex-wrap">
              {/* Na czas — dla bieżni, stepmilla, ergometru. Przy tym trybie
                  serie i ciężar znikają, bo nie ma czego w nie wpisać. */}
              <div className="flex gap-2 items-center">
                <label className="text-xs text-gray-500">Na czas</label>
                <button type="button"
                  onClick={() => setEntries(prev => prev.map(e =>
                    e.key === entry.key
                      ? { ...e, timed: !e.timed, customSets: false, bodyweight: false }
                      : e
                  ))}
                  className={`relative inline-flex h-5 w-10 items-center rounded-full transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 ${entry.timed ? 'bg-orange-500 hover:bg-orange-600' : 'bg-gray-200 hover:bg-gray-300'}`}>
                  <span className={`inline-block h-4 w-4 rounded-full bg-white shadow transition-transform ${entry.timed ? 'translate-x-5' : 'translate-x-1'}`} />
                </button>
              </div>
              {!entry.timed && (
              <div className="flex gap-2 items-center">
            ...
            {entry.timed ? (
              <div>
                ...
                <input
                  ...
                  value={entry.durationMin === 0 ? '' : entry.durationMin}
                  placeholder="np. 20"
                  onChange={e => setEntries(prev => prev.map(x =>
                    x.key === entry.key ? { ...x, durationMin: parseInt(e.target.value, 10) || 0 } : x
                  ))}
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-base text-gray-900 text-center" />
                <div className="flex gap-2 mt-2">
                  {[10, 15, 20, 30].map(m => (
                    <button key={m} type="button"
                      onClick={() => setEntries(prev => prev.map(x =>
                        x.key === entry.key ? { ...x, durationMin: m } : x
                      ))}
                      className="flex-1 py-1.5 rounded-lg bg-gray-100 text-sm text-gray-700">
                      {m}
                    </button>
                  ))}
                </div>
