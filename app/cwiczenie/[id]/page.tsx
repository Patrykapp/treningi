...
export default function CwiczeniePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Ćwiczenia mierzone czasem nie mają ciężaru ani powtórzeń — wchodząc do
  // rekordów robiłyby z „1 powt. @ 0 kg" wynik, a z wykresu ciężaru schodki do zera.
  const liftEntries = entries.filter(e => !(e.durationSec && e.durationSec > 0));
  const timedEntries = entries.filter(e => e.durationSec && e.durationSec > 0);
  const isTimedExercise = timedEntries.length > 0 && liftEntries.length === 0;
  const bestMin = timedEntries.length
    ? Math.max(...timedEntries.map(e => Math.round((e.durationSec ?? 0) / 60))) : 0;
  const bestWeight = liftEntries.length ? Math.max(...liftEntries.map(calcMax)) : 0;
  const isBodyweightExercise = liftEntries.length > 0 && bestWeight === 0;
  // Najlepsze szacowane 1RM z historii (Epley, per seria)
  const best1RM = liftEntries.length ? Math.max(...liftEntries.map(e => {
  ...
  const chartData = buildChart(liftEntries, compareEntries.filter(e => !(e.durationSec && e.durationSec > 0)), chartType);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20">
      ...
      <div className="px-4 py-4 space-y-4 max-w-2xl mx-auto md:max-w-3xl lg:max-w-4xl">
        ...
        {/* Przy ćwiczeniu mierzonym czasem rekordem jest najdłuższy czas, a nie
            ciężar — kafelki „rekord kg" i „1RM" nie miałyby tu czego pokazać. */}
        {entries.length > 0 && isTimedExercise ? (
          <div className="grid gap-2 grid-cols-3">
            <div className="bg-white rounded-2xl p-3 text-center shadow-sm">
              <div className="text-xl font-bold text-orange-600">{bestMin}</div>
              <div className="text-xs text-gray-500">rekord min</div>
            </div>
            <div className="bg-white rounded-2xl p-3 text-center shadow-sm">
            ...
            <div className="bg-white rounded-2xl p-3 text-center shadow-sm">
              <div className="text-xl font-bold text-gray-900">
                {lastEntry?.durationSec ? Math.round(lastEntry.durationSec / 60) : '-'}
              </div>
              <div className="text-xs text-gray-500">ostatnio min</div>
            </div>
          </div>
        ) : entries.length > 0 && (
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="divide-y divide-gray-100">
              {entries.map(entry => {
                ...
                const timed = Boolean(entry.durationSec && entry.durationSec > 0);
                ...
                return (
                  <div key={entry.id} className="px-4 py-3">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-2">
                        {timed
                          ? <span className="text-sm font-bold text-orange-600">{Math.round((entry.durationSec ?? 0) / 60)} min</span>
